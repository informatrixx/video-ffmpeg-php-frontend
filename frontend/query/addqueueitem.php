<?php
	...
	$aDataError = false;
	if(!isset($_GET['outfolder']) || !is_dir($_GET['outfolder']))
	{
		$aResult['error'] = 'Invalid "outfolder"!';
		$aDataError = true;
	}
	else
	{
		$aOutFolderHistory = json_decode(json: file_get_contents(ROOT . 'config/outfolder_history.json'), associative: true);
		array_unshift($aOutFolderHistory, $_GET['outfolder']);
		$aOutFolderHistory = array_unique($aOutFolderHistory);
		if(count($aOutFolderHistory) > 10)
			array_pop($aOutFolderHistory);
		
		file_put_contents(filename: ROOT . 'config/outfolder_history.json', data: json_encode(value: $aOutFolderHistory, flags: JSON_PRETTY_PRINT));
	}
