<?php
	...
	$aOutFileJPG = rtrim(__DIR__, '/') . '/cache/scan-' . md5($_GET['file']) . '-' . $aSeek . '.jpg';
	$aTmpFileBMP = rtrim(__DIR__, '/') . '/cache/scan-' . md5($_GET['file']) . '-' . $aSeek . '.bmp';
	...
	else
	{
		...
		$aReturn = array(
			...
			'sar'	=>		$aSAR,
			'seekHuman' =>	"$aHours:$aMinutes:$aSeconds",
			'crop'	=>		isset($aCropData) ? $aCropData : null,
			);
	}
